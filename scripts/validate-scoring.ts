...
console.log('');
console.log(
  'Hour  Sun-alt  Sun-az  Score   Label              Notes',
);
console.log(
  '────  ───────  ──────  ─────   ─────────────────  ─────',
);
console.log('');
for (let h = 12; h <= 21; h++) {
  const r = computeSunScore(kiebert, h, DATE, 'sunny');
  ...
  console.log(
    `${h.toString().padStart(4)}:00  ${altDeg}°  ${azDeg}°   ${score}   ${label}  ${notes}`,
  );
}
...
let sum = 0;
for (let h = 15; h <= 19; h++) {
  sum += computeSunScore(kiebert, h, DATE, 'sunny').score;
}
const avg = sum / 5;
console.log(
...
const scored = TERRACES.map((t) => {
  const r = computeSunScore(t, 16, DATE, 'sunny');
  return {
    id: t.id,
    name: t.name,
    facing: t.facing,
...
for (const s of nearby.slice(0, 10)) {
  console.log(
    `  ${s.score.toFixed(3)}  ${s.name.padEnd(35).slice(0, 35)}  ${s.area}`,
  );
}
