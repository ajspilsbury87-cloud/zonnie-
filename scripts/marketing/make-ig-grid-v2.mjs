...
async function tile01_hero() {
  ...
  await stack(layers, {
    ...
    sub: '1,900+ Amsterdam terraces, scored for sun — live, every hour.',
  });
...
async function socialCard() {
  ...
  const sub = await text({ markup: `<span foreground="${C.inkSoft}">1,900+ Amsterdam terraces, scored for sun — live.</span>`, font: 'Inter 22', fontfile: I_REG, align: 'left' });
